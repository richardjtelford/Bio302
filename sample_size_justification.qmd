...
## Repeating functions many times
...
out <- 0
for(i in 2:nrep) {
  out <- c(out, out[i-1] + rnorm(1))
}
```

## The apply family

-   `apply` iterates over the rows/columns of a data.frame/matrix

-   `lapply` iterates over a list/data.frame and returns a list

-   `sapply` same as `lapply` but simplifies to a vector

Generally better to use purrr for consistent behaviour.

## purrr package

-   `map` iterates over a vector/list and returns a list
...
```{r}
rnd <- 1:10 |> 
  map(\(x)rnorm(x)) # anonymous function
```

-   `map2` and `pmap` for iterating over multiple inputs

-   `map_chr`/`map_dbl` etc simplify list to a vector

-   pipe to `list_rbind()` to combine data.frames
...
plan(sequential)
tic()
nothing <- future_map(sequence, Sys.sleep)
toc()

plan(strategy = multisession, workers = no_cores)
tic()
nothing <- future_map(sequence, Sys.sleep)
toc()
```
...
step_size <- 30
n_steps <- 30
steps <- seq(step_size, by = step_size, length.out = n_steps)

sim <- map(seq_len(n_experiments), \(i){
 x <- rnorm(n = step_size * n_steps) 
 y <- rnorm(n = step_size * n_steps)
 res <- steps |> 
   map(\(step) {
   p <- t.test(x[1:step], y[1:step])$p.value
   c(i = i, step = step, p = p)
 }) |> 
   bind_rows()
 if(any(res$p < 0.05)) {
   first_sig <- which(res$p < 0.05)[1]
   res <- res[1:first_sig, ]
 }
 res
}) |> 
...
sim |> group_by(step) |> 
  summarise(n = sum(p < 0.05), .groups = "drop") |> 
  mutate(n2 = cumsum(n)/n_experiments) |> 
  ggplot(aes(x = step, y = n2)) +
